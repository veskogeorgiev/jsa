package jsa.test.port.bc;

import jsa.annotations.API;
import jsa.annotations.API.Version;
import jsa.annotations.APIPort;
import jsa.annotations.APIPortType;
import jsa.endpoint.thrift.annotations.ExposeThrift;
import jsa.test.backward.v2.BackwardCompAPI;
import jsa.test.backward.v2.BackwardCompAPI.Iface;

import org.apache.thrift.TException;

@API(name = "tdemo", version = @Version(number = 2, tag = "v2"))
@ExposeThrift(module = BackwardCompAPI.class)
@APIPort(api = BackwardAPIv2.class, context = "t", type = APIPortType.DECORATOR)
public class BackwardAPIv2 implements Iface {

    @Override
    public void save(String name1) throws TException {
        System.out.println("v2: " + name1);
    }

}
